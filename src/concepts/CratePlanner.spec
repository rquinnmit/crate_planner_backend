<concept_spec>
concept CratePlanner [Track, Prompt] -> Implementation in crate_planner.ts, Testing in crate_planner.test.ts
...
a set of Prompts with
	...
	an optional targetDuration Number
...
a set of DerivedIntents with
	...
	an energyCurve String
	a targetEnergy Number
	a minPopularity Number
	a targetKeyCamelot CamelotKey

a set of SpotifyQueryPlans with
	a list of searchQueries List<String>
	a list of seedGenres List<String>
	a list of seedArtists List<String>
	a list of seedTracks List<String>
	a tunables
	a reasoning String
...
deriveIntentLLM(prompt: Prompt, seedTracks: List<TrackId>, llm: GeminiLLM): (intent: DerivedIntent)
...
generateCandidatePool(intent: DerivedIntent, llm: GeminiLLM): (pool: CandidatePool)
requires intent is valid
effect uses the intent and an LLM to produce a set of track candidates

generateCandidatePoolWithSpotify(intent: DerivedIntent, llm: GeminiLLM): (pool: CandidatePool)
requires intent is valid
effect searches spotify for tracks matching the intent

sequencePlan(intent: DerivedIntent, pool: CandidatePool, seedTracks: List<TrackId>?, llm: GeminiLLM): (plan: Plan)
requires pool is nonempty
effect returns a plan with an ordered track list and duration

explainPlan(plan: Plan, llm: GeminiLLM): (annotated: Plan)
requires true
effect calls an LLM to generate human-readable annotations for the generated crate

revisePlan(plan: Plan, instructions: String, llm: GeminiLLM): (revised: Plan)
...
getReplacementTracks(plan: Plan, maxTracks: number)
requires true
effect gets smart replacements for tracks during the revision phase
